...
```
note_keeper_bot/
├── bot.py           # Telegram-интеграция: команды, кнопки, диалоги, напоминания
...
Код разделён на три небольших модуля вместо одного файла: `parsing.py` не зависит ни от
чего в проекте, `db.py` зависит только от `parsing.py` (для форматирования дат в CSV), а
`bot.py` — единственное место с интеграцией Telegram. Циклических импортов нет.
