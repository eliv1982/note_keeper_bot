...
def _make_category_with_note(bot, user_id=USER_A, cat_name="Дом", text="Купить молоко"):
    cat_id, err = bot.create_category(user_id, cat_name)
    assert err is None
    note_id, err = bot.add_note(cat_id, user_id, text)
    assert err is None
...
def test_create_category_and_add_note(isolated_db):
    bot = isolated_db
    cat_id, note_id = _make_category_with_note(bot)
    rows, err = bot.get_notes_by_category_and_user(cat_id, USER_A)
    assert err is None
...
def test_duplicate_category_name_rejected(isolated_db):
    bot = isolated_db
    bot.create_category(USER_A, "Дом")
    cat_id, err = bot.create_category(USER_A, "Дом")
...
def test_same_category_name_allowed_for_different_users(isolated_db):
    bot = isolated_db
    id_a, err_a = bot.create_category(USER_A, "Дом")
    id_b, err_b = bot.create_category(USER_B, "Дом")
...
def test_update_note(isolated_db):
    bot = isolated_db
    cat_id, note_id = _make_category_with_note(bot)
    ok, err = bot.update_note(cat_id, note_id, USER_A, "Купить хлеб")
    assert ok is True and err is None
    rows, _ = bot.get_notes_by_category_and_user(cat_id, USER_A)
...
def test_delete_category_removes_its_notes(isolated_db):
    bot = isolated_db
    cat_id, note_id = _make_category_with_note(bot)
    ok, err = bot.delete_category(USER_A, cat_id)
    assert ok is True and err is None
    assert bot.get_category_by_id_and_user(cat_id, USER_A) is None
...
def test_read_notes_denied_for_wrong_user(isolated_db):
    bot = isolated_db
    cat_id, _ = _make_category_with_note(bot, user_id=USER_A)
    rows, err = bot.get_notes_by_category_and_user(cat_id, USER_B)
    assert rows is None
...
def test_update_note_denied_for_wrong_user(isolated_db):
    bot = isolated_db
    cat_id, note_id = _make_category_with_note(bot, user_id=USER_A)
    ok, err = bot.update_note(cat_id, note_id, USER_B, "Подмена текста")
    assert ok is False
    assert err is not None
...
def test_delete_category_denied_for_wrong_user(isolated_db):
    bot = isolated_db
    cat_id, _ = _make_category_with_note(bot, user_id=USER_A)
    ok, err = bot.delete_category(USER_B, cat_id)
    assert ok is False
    assert err is not None
    # Категория осталась у настоящего владельца
    assert bot.get_category_by_id_and_user(cat_id, USER_A) is not None
...
def test_delete_note_by_wrong_user_does_not_delete_it(isolated_db):
    ...
    bot = isolated_db
    cat_id, note_id = _make_category_with_note(bot, user_id=USER_A)
    ok, err = bot.delete_note(cat_id, note_id, USER_B)
    assert ok is True
    assert err is None
    # Настоящий владелец всё ещё видит заметку — данные не были удалены чужим пользователем.
    note = bot.get_note_by_id_and_user(cat_id, note_id, USER_A)
    assert note is not None


def test_forged_nonexistent_category_id(isolated_db):
    bot = isolated_db
    assert bot.get_category_by_id_and_user(999999, USER_A) is None
    note_id, err = bot.add_note(999999, USER_A, "текст")
...
def test_delete_note_is_idempotent(isolated_db):
    bot = isolated_db
    cat_id, note_id = _make_category_with_note(bot)
    ok1, err1 = bot.delete_note(cat_id, note_id, USER_A)
    assert ok1 is True and err1 is None
    assert bot.get_note_by_id_and_user(cat_id, note_id, USER_A) is None
    ...
    ok2, err2 = bot.delete_note(cat_id, note_id, USER_A)
    assert ok2 is True
    assert err2 is None
