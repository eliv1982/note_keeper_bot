...
def test_explicit_date_and_time():
    due_utc, note = parsing.parse_due_message("15.03.2026 13:00 Тренировка")
...
def test_regression_zavtra_v_time_with_colon():
    ...
    due_utc, note = parsing.parse_due_message("Тренировка завтра в 10:00")
    assert note == "Тренировка"
...
def test_zavtra_v_time_without_colon():
    due_utc, note = parsing.parse_due_message("Купить молоко завтра в 10")
...
def test_segodnya_with_colon_time():
    due_utc, note = parsing.parse_due_message("Позвонить сегодня 18:30")
...
def test_no_date_or_time_returns_none():
    due_utc, note = parsing.parse_due_message("Просто текст без даты")
    assert due_utc is None
    assert note is None


def test_invalid_hour_returns_none():
    due_utc, note = parsing.parse_due_message("Текст в 25:00 завтра")
    assert due_utc is None
    assert note is None


def test_empty_text_after_stripping_returns_none():
    due_utc, note = parsing.parse_due_message("завтра 10:00")
    assert due_utc is None
    assert note is None


def test_parse_due_datetime_to_utc():
    assert parsing.parse_due_datetime_to_utc("15.03.2026", "13:00") == "2026-03-15 10:00:00"


def test_parse_due_datetime_to_utc_invalid():
    assert parsing.parse_due_datetime_to_utc("31.02.2026", "13:00") is None


def test_format_created():
    assert parsing.format_created("2026-03-15 10:00:00") == "15-03-2026 13:00"
